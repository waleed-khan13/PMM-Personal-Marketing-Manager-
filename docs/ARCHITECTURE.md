...
- LinkedIn Company Page publishing remains separate from member publishing because its authorization boundary is organization-specific and access gated. The connector requires `w_organization_social` plus `rw_organization_admin`, binds the token to the configured OIDC member, and checks `ORGANIC_SHARE_CREATE` through `/rest/organizationAuthorizations` for the exact `urn:li:organization:{id}`. Only a verified connector can send a frozen approved `linkedin-company` revision through `/rest/posts`; LinkedIn product approval, Page roles, and scope grants remain external prerequisites that LocalGrowth never attempts to bypass.
- WhatsApp Cloud is an outbound, notification-only connector in strict-localhost mode. Its health check binds an encrypted permanent token to the configured business Phone Number ID. Draft generation sends an operator-approved template with four ordered text parameters—channel, title, revision, and bounded excerpt—and records the returned message ID. The recipient and approved template are explicit operator configuration, not model-selected destinations.
- Connectors that mandate a public inbound webhook, including interactive WhatsApp Cloud callbacks and delivery-status webhooks, remain optional or outbound-only in strict localhost mode. WhatsApp decisions therefore stay in the local UI, Telegram, or Slack.
