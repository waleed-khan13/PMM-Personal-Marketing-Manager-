...
| Image AI | OpenAI-compatible Images API + Automatic1111/Forge | User-selected hosted or fully local image generation with a shared review boundary. |
...
Image generation has a separate singleton `image_provider_settings` row so its encrypted credential, model, and endpoint never overwrite the text provider. The hosted adapter sends a single prompt to an operator-selected OpenAI-compatible `/v1/images/generations` endpoint and expects base64 image data. The local adapter sends a bounded `txt2img` request to Automatic1111/Forge and may temporarily override a configured checkpoint for that request. `media_generations` records every successful generation, including duplicate outputs, while `media_assets` carries the primary non-secret prompt, provider, model, and parameters displayed for review. No generation route creates a post or invokes a publisher.
